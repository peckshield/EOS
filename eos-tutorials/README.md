...
### 理解EOS链上特性
了解EOS作为区块链的基本属性，也需要理解EOS拥有的特性：
- 链上数据透明
- EOS智能合约可随时更新
- EOS智能合约中的函数没在ABI文件中声明，同样可能被外部调用
### 具有异常检测与处理逻辑
智能合约不会总是按照预想的逻辑运行，要检测可能出现的异常，及时处理甚至中止合约运行。
- 合理使用 eosio_assert ，不符合预期时可及时停止执行
- 合理限制资金的转入转出金额及次数，避免异常操作
### 完整的测试发布流程
严谨地对待智能合约的开发、测试、发布、维护等生命周期。
- 开发中避免引入已发现的安全问题
- 测试时谨慎部署到主网，避免测试版本被攻击
- 发布后保持对相关安全事件的关注，及时检查
- 维护过程也需要完整的测试，避免引入新的问题
### 基本的应急响应能力
要有基本的应急响应流程，降低出现问题时带来的危害
- 获取并关注合约的运行数据，也可使用第三方的安全预警服务，及时发现异常行为
- 合约中添加类似暂停合约服务的功能，在出现安全事件时能及时停止服务
...
```
#### 攻击实例
[探究随机数漏洞背后的技术原理：EOS.WIN竞猜游戏是如何被攻破的？](https://mp.weixin.qq.com/s/WTiwaIbB7N2VDKKgcXo0Fg)